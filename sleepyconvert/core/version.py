...
entrypoint (see ``run``), so it works even when it appears alongside the
input/output paths (e.g. ``sleepyconvert a.csv b.parquet --version``) — the rest
of the command is ignored. The update check is best-effort and never fails the CLI.
"""
