...
How to publish `sleepyconvert` to PyPI with [Poetry](https://python-poetry.org/).

## Prerequisites

- Poetry installed (`uv tool install poetry`).
- A PyPI account with maintainer access to the `sleepyconvert` project.
...
5. Verify the release:
   ...
   ```sh
   uv tool install --upgrade sleepyconvert
   ```
